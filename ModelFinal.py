...
class SignModel:
    def __init__(self) :
        ...
        self.model = load_model(r"D:\PYTHON\Final_Year\signer\cnn8grps_rad1_model.h5")
    ...
    def predict(self,input_image, img_keypoints):

        self.test_image = input_image
        self.pts = img_keypoints
        
        self.white=self.test_image
        self.white = self.white.reshape(1, 400, 400, 3)
        prob = np.array(self.model.predict(self.white)[0], dtype='float32')
        ...
        # con for [d][pqz]
        fg = 19
        # #("_________________ch1=",ch1," ch2=",ch2)
        ...
        if ch1 == 1:
            ...
            if (self.pts[6][1] > self.pts[8][1] and self.pts[10][1] > self.pts[12][1] and self.pts[14][1] < self.pts[16][1] and self.pts[18][1] < self.pts[20][
                ...
                ch1 = 'K'
            #print(self.distance(self.pts[6], self.pts[10]))
            #print(self.distance(self.pts[8], self.pts[12]))
...
class HandDetect:
    def __init__(self,image) -> None:
        ...
        try:
            ...
            self.cv2image = cv2.flip(self.cv2image , 1)

            self.hands = hd.findHands(self.cv2image, draw=False, flipType=True)


            self.cv2image_copy = np.array(self.cv2image)
    ...
    def detector(self):
        if self.hands:
                ...
                try:
                    ...
                    #print(hand)
                    x, y, w, h = hand['bbox']
                    image = self.cv2image_copy[y - offset:y + h + offset, x - offset:x + w + offset]

                    white = cv2.imread(r"D:\PYTHON\Final_Year\signer\white.jpg")
                    # img_final=img_final1=img_final2=0

                    handz = hd2.findHands(image, draw=False, flipType=True)
                    #print("image:____________",image)
                    #print(" ", self.ccc)
                    #self.ccc += 1
                    if handz:
                        hand = handz[0][0]
